...
fn add_node(&mut self, name: &str, node_id: Option<usize>, append_to_node: bool, update: bool) -> TreeNode {
    ...
    if let Some(node_id) = node_id {
        if let Some(node) = self.find_node_by_id(node_id) {
            if append_to_node {
                ...
                let new_parent = TreeNode::new_branch(name, new_id, Some(vec![new_leaf.clone()]));
...
fn add_node(node: &rocket::State<Arc<Mutex<Tree>>>, new_node: Json<TreeNode>) -> Json<Tree> {
    ...
    let node_id = node_to_insert.children.as_ref().and_then(|children| {
        children.get(0).map(|child| child.id)
    });
...
async fn main() {
 ...
 let mut initial_tree = build_initial_tree();
    ...
    let rocket = rocket::build().attach(cors.to_cors().unwrap()).mount("/", routes![get_tree, add_node]).manage(arc_tree);
    rocket.launch().await;
}
